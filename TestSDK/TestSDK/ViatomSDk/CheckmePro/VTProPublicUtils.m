//
//  VTProPublicUtils.m
//  libCheckme
//
//  Created by 李乾 on 15/4/27.
//  Copyright (c) 2015年 Viatom. All rights reserved.
//

#import "VTProPublicUtils.h"



@implementation VTProPublicUtils

const unsigned char Table_Pro_CRC8[256]={      /*CRC8 ±í*/
    
};

+ (uint8_t)calCRC8:(unsigned char *)RP_ByteData bufSize:(unsigned int)Buffer_Size
{
    uint8_t x,R_CRC_Data;
    uint32_t i;
    
    R_CRC_Data=0;
    for(i=0;i<Buffer_Size;i++)
    {
        x = R_CRC_Data ^ (*RP_ByteData);
        R_CRC_Data = Table_Pro_CRC8[x];
        RP_ByteData++;
    }
    return R_CRC_Data;
}

+ (NSString *)makeDateFileName:(NSDateComponents *)date fileType:(VTProFileType)type {
    NSString* dateString = [NSString stringWithFormat:@"%04ld%02ld%02ld%02ld%02ld%02ld",(long)date.year,(long)date.month,(long)date.day,(long)date.hour,(long)date.minute,(long)date.second];
    if (type == VTProFileTypeEcgVoice) {
        NSString* voiceString = [NSString stringWithFormat:@"%@.wav",dateString];
        return voiceString;
    }
    return dateString;
}

@end
